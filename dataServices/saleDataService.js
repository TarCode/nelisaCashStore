module.exports = function (connection) {
  ...
  this.getAllSales = function (cb) {
      getData('SELECT sale_id, prod_name, DATE_FORMAT(date, "%d/%l/%Y") as date, qtySold, salePrice from sales, product WHERE sales.prod_id = product.prod_id order by sale_id desc', cb );
  };//done
  ...
  this.getUpdateSale = function (data, cb) {
      insertData('SELECT sale_id, prod_id, DATE_FORMAT(date, "%d/%l/%Y") as date, qtySold, salePrice from sales WHERE sale_id = ?', data, cb );
  };
